...
#ifndef PARAM_NB_SAVED_SI
#define PARAM_NB_SAVED_SI 17
#endif
 ...
 */
static void sig_perk_gen_first_response(sig_perk_hash_state_t *h2_state, digest_t h1,
                                        perk_s_i_t saved_s_i[PARAM_NB_SAVED_SI], const perm_t pi_0[PARAM_TAU],
    ...
    for (int e = 0; e < PARAM_TAU; ++e) {
        ...
        if (e >= (PARAM_TAU - PARAM_NB_SAVED_SI)) {
            memcpy(saved_s_i[e - (PARAM_TAU - PARAM_NB_SAVED_SI)].s_i[0], (uint8_t *)s_i, sizeof(vect1_t));
        }
        ...
        for (int i = 1; i < PARAM_N; ++i) {
            ...
            if (e >= (PARAM_TAU - PARAM_NB_SAVED_SI)) {
                memcpy(saved_s_i[e - (PARAM_TAU - PARAM_NB_SAVED_SI)].s_i[i], (uint8_t *)s_i, sizeof(vect1_t));
            }
        }
    }
 ...
 */
static void sig_perk_gen_second_response(perk_signature_t *signature, const perk_s_i_t saved_s_i[PARAM_NB_SAVED_SI],
                                         const seed_t mseed, const challenge_t challenges[PARAM_TAU],
                                         const vect1_t x[PARAM_T], const perm_t pi_0[PARAM_TAU]) {
    sig_perk_prg_state_t prg_state;
    ...
    for (uint8_t e = 0; e < (PARAM_TAU - PARAM_NB_SAVED_SI); ++e) {
        vect1_t tmp;
        vect1_t v_i;  // TODO:  tmp and v_i can share memory??
        vect1_t s_0;
        vect1_t s_i;
    ...
    for (uint8_t e = (PARAM_TAU - PARAM_NB_SAVED_SI); e < PARAM_TAU; ++e) {
        perk_theta_seeds_tree_t theta_tree;
        const uint16_t alpha = challenges[e].alpha;
        ...
        memcpy(signature->responses[e].z1, saved_s_i[e - (PARAM_TAU - PARAM_NB_SAVED_SI)].s_i[alpha - 1],
               sizeof(vect1_t));
        if (alpha != 1) {
            memcpy(signature->responses[e].z2_pi, pi_0[e], sizeof(perm_t));
        } else {
...
uint8_t sig_perk_sign(perk_signature_t *signature, const perk_private_key_t *sk, const uint8_t *message_bytes,
    ...
    perk_s_i_t saved_s_i[PARAM_NB_SAVED_SI] = {0};
    ...
    sig_perk_gen_first_response(&h2_state, signature->h1, saved_s_i, (const perm_t *)pi_0, mseed, signature->salt,
                                challenges, (const vect1_t *)pk.x);

    sig_perk_gen_second_challenge_s(signature->h2, challenges, &h2_state);
    sig_perk_gen_second_response(signature, saved_s_i, mseed, challenges, (const vect1_t *)pk.x, (const perm_t *)pi_0);
